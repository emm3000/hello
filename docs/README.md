...
- `AI_BACKEND_PLAN.md` — Status: **In progress**. Moves every AI call behind one Supabase Edge Function with a provider chain, a shared note cache, guest identity and a daily allowance; six phases, each with its falsifier. Phases 1 and 2 are implemented.
