...
#### S2-T7: Retry budget under the free-tier daily quota
- **Files:** `data/src/main/kotlin/com/emm/data/flashcard/GeminiService.kt`, `app/src/main/kotlin/com/emm/hello/enrichment/FlashcardEnrichmentWorker.kt`, `data/src/main/kotlin/com/emm/data/flashcard/DailyGenerationQuota.kt`, `data/src/main/kotlin/com/emm/data/flashcard/Prompt.kt`
- **Why:** measured on 2026-09-06: `gemini-2.5-flash-lite` on the free tier returns `429 Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_requests, limit: 20` — twenty requests per day, reset at midnight Pacific. `GeminiService` retries every failure four times, including a `DomainValidationException` for a well-formed answer the policies reject, and the worker re-runs the whole thing three times, so one word the model keeps getting wrong costs up to 12 of the 20. `DailyGenerationQuota.DEFAULT_LIMIT` is 50, so the app-side guard never fires first. The trigger was "give up": every response left `usage_pattern` empty although the prompt asks for it on `phrasal_verb`; the prompt now says it in stronger terms and lists the per-type required fields right before `quality_checks`, unverified against Gemini at the time of writing.
- **What to do:** retry only transient failures (timeout, 5xx, 429 with backoff), never `DomainValidationException`; on a validation failure do at most one regeneration that feeds the issue codes back to the model; align `DailyGenerationQuota` with the measured limit; verify the "give up" capture ends `ENRICHED` on device. `FlashcardEnrichmentWorker` now logs the validation issue codes and `CrashlyticsGeminiTelemetry` logs the raw failed response locally, so the failing field is visible in logcat without Crashlytics.
- **Criterion:** a capture whose first answer fails validation costs at most 2 requests, and a phrasal verb enriches on device within the daily budget.
- **Estimate:** 3 h.
- **Status:** [x] — shipped 2026-09-06: `GeminiService` now classifies each failure with a private `isTransient()` check before retrying. Only `TimeoutCancellationException`, `java.io.IOException`, and the Firebase AI `RequestTimeoutException`, `ServerException`, `QuotaExceededException`, `UnknownException` are retried with the existing backoff, except a `ServerException` whose message mentions App Check (the SDK maps the App Check 403 `Firebase App Check token is invalid.` to a plain `ServerException` with no status code, so the message is the only signal), which is thrown after the first attempt since 2026-09-06 to stop burning up to 4 in-process calls on a token the server will keep rejecting; every other error, including `DomainValidationException`, `kotlinx.serialization.SerializationException`, `IllegalArgumentException` and `AmbiguousGenerationInputException` from the parser, is thrown after the first attempt with no delay. `processLearningNoteWithParser` no longer rethrows a timeout as a non-retryable `CancellationException` before checking whether it is transient, and telemetry now reports the real number of attempts made instead of always reporting the full retry budget. `EnrichCapturedFlashcardUseCase` does at most one regeneration: when the base input fails validation (either the repository throws `DomainValidationException` or `requireValid()` does after the call returns), it retries exactly once with `FlashcardGenerationInput.previousIssues` set to the failed issues, and `Prompt.buildLearningNotePrompt` appends a feedback section listing each rejected `code (field: ...)` when `previousIssues` is non-empty. `FlashcardEnrichmentWorker` now keeps the caught error and asks the new `EnrichmentRetryPolicy`: a `DomainValidationException` or `GenerationQuotaExceededException` marks the card `FAILED` immediately (`Result.failure()`) instead of consuming the worker's own 3-attempt retry budget; every other error still goes through `Result.retry()` up to `MAX_ATTEMPTS = 3`. `DailyGenerationQuota.DEFAULT_LIMIT` is now 20, matching the measured free-tier cap, and `RepositoryModule` anchors the quota's day boundary to `ZoneId.of("America/Los_Angeles")` so it resets with the actual Gemini quota instead of the device's local zone. Worst case per capture is now 2 requests (one regeneration) instead of up to 12. `:domain:test`, `:data:testDebugUnitTest` and detekt are green. Device verification on `medium_phone` (2026-09-06 19:22 Lima, build `afe5aa7`): tapping Retry on the `FAILED` "give up" card enriched it in one request (`Worker result SUCCESS` after 6 s, no `enrich:error` and no parse-failure telemetry line), `usagePattern` = `give up + [on + noun / -ing / clause]`, `type` = `PhrasalVerb`. The stronger prompt from `6a152d5` was enough; the regeneration path was not exercised on device and is covered by `EnrichCapturedFlashcardUseCaseTest`.
- **Depends on:** S2-T2.
...
- **Status:** [x] — `UpdateFlashcardUseCase` decided and shipped 2026-09-06: it now owns the manual-completion rule. After `repository.update`, when the saved meaning is non-blank it reads the card back and, only if `enrichmentStatus` is `FAILED`, sets it to `ENRICHED`, so a card the model kept rejecting becomes studiable once the user fills the meaning from Edit Flashcard (`meaning` is now an editable field there). `PENDING` and `ENRICHED` are never rewritten and a blank meaning never touches the status. `DefaultFlashcardRepository.update` no longer rejects a blank meaning (it used to throw for any unenriched card, which the ViewModel tests never saw because they run on a fake repository). Pinned by `UpdateFlashcardUseCaseTest`, `DefaultFlashcardRepositoryUpdateTest` and `EditFlashcardViewModelTest`. `SoftDeleteFlashcardUseCase` was deleted on 2026-09-06 and both `EditFlashcardViewModel` and `FlashcardDetailViewModel` now call `FlashcardRepository.softDeleteFlashcard` directly: it carried no logic (the not-found and already-deleted invariants live in `DefaultFlashcardRepository`) and both ViewModels already held `FlashcardRepository`.
