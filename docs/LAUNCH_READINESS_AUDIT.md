...
- **Status:** [~] — `UpdateFlashcardUseCase` decided and shipped 2026-09-06: it now owns the manual-completion rule. After `repository.update`, when the saved meaning is non-blank it reads the card back and, only if `enrichmentStatus` is `FAILED`, sets it to `ENRICHED`, so a card the model kept rejecting becomes studiable once the user fills the meaning from Edit Flashcard (`meaning` is now an editable field there). `PENDING` and `ENRICHED` are never rewritten and a blank meaning never touches the status. `DefaultFlashcardRepository.update` no longer rejects a blank meaning (it used to throw for any unenriched card, which the ViewModel tests never saw because they run on a fake repository). Pinned by `UpdateFlashcardUseCaseTest`, `DefaultFlashcardRepositoryUpdateTest` and `EditFlashcardViewModelTest`. `SoftDeleteFlashcardUseCase` is still undecided.
