...
private const val EXPECTED_JOB_INTERVIEW_NOTE_COUNT = 25
private const val JOB_INTERVIEW_NOTE_ID_PREFIX = "curated.job-interview."
private val catalogKey: Regex = Regex("^[a-z0-9.-]+$")

class JobInterviewDeckTest {

    private val validate = ValidateGeneratedLearningNoteUseCase()
    private val notes: List<GeneratedLearningNote> = JobInterviewDeck.deck.notes
    ...
    fun `the deck has exactly twenty-five notes`() {
        assertEquals(EXPECTED_JOB_INTERVIEW_NOTE_COUNT, notes.size)
    }
    ...
    fun `every note id carries the deck prefix`() {
        val offenders: List<String> = notes.map(GeneratedLearningNote::noteId)
            .filterNot { it.startsWith(JOB_INTERVIEW_NOTE_ID_PREFIX) }
