...
private const val EXPECTED_DAILY_STANDUP_NOTE_COUNT = 25
private const val DAILY_STANDUP_NOTE_ID_PREFIX = "curated.daily-standup."
private val catalogKey: Regex = Regex("^[a-z0-9.-]+$")

class DailyStandupDeckTest {

    private val validate = ValidateGeneratedLearningNoteUseCase()
    private val notes: List<GeneratedLearningNote> = DailyStandupDeck.deck.notes
    ...
    fun `the deck has exactly twenty-five notes`() {
        assertEquals(EXPECTED_DAILY_STANDUP_NOTE_COUNT, notes.size)
    }
    ...
    fun `every note id carries the deck prefix`() {
        val offenders: List<String> = notes.map(GeneratedLearningNote::noteId)
            .filterNot { it.startsWith(DAILY_STANDUP_NOTE_ID_PREFIX) }
    ...
    @Test
    fun `every note names the moment in the standup`() {
        val offenders: List<String> = notes
            .filterNot { it.sourceContext.startsWith("Daily: ") }
            .map(GeneratedLearningNote::noteId)
