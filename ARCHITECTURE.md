...
Before Koin, `App.installAppCheck()` installs the Firebase App Check provider: the debug provider in `debug` builds, Play Integrity in `release`. The Hello backend enforces App Check on every Edge Function call, so each request carries an attestation token from `AppCheckTokenProvider`; the token is fetched lazily on the first AI call, never during startup.
...
- a zero-due day can ask for a situation and ~6 candidate words to add, gated behind `BuildConfig.USE_CANNED_AI` (canned suggestions in debug, the Edge Function backend in release)
