drop function if exists reserve_generation(uuid, text, text, int, int, timestamptz);
  ...
  select
    count(*) filter (
      where outcome = 'success'
         or (outcome = 'pending' and created_at >= p_stale_before)
    ),
...
revoke execute on function reserve_generation(uuid, text, text, int, int, timestamptz, timestamptz) from public;
revoke execute on function reserve_generation(uuid, text, text, int, int, timestamptz, timestamptz) from anon;
revoke execute on function reserve_generation(uuid, text, text, int, int, timestamptz, timestamptz) from authenticated;
grant execute on function reserve_generation(uuid, text, text, int, int, timestamptz, timestamptz) to service_role;
