...
export function buildLearningNotePrompt(input: GenerateNoteRequest): string {
  const base: string =
...
If the input is too ambiguous or unusable, return success=false with a short error.message explaining why. Do not include markdown or any text outside the JSON.`;
  ...
  return base + "\n\n" + feedbackSection(input.previous_issues);
}

function feedbackSection(
  previousIssues: { code: string; field: string }[],
): string {
  const issueLines: string = previousIssues
    .map((issue: { code: string; field: string }): string =>
      `- ${issue.code} (field: ${issue.field})`
    )
    .join("\n");
...
export function buildWordSuggestionPrompt(recentWords: string[]): string {
  const levelContext: string = buildLevelContext(recentWords);
  return `You are an English tutor for a Spanish-speaking learner.
${levelContext}
...
function buildLevelContext(recentWords: string[]): string {
...
export function withSchemaAppendix(
  prompt: string,
  jsonSchema: Record<string, unknown>,
): string {
  return prompt + "\n\n" + SCHEMA_APPENDIX_HEADER + "\n" +
